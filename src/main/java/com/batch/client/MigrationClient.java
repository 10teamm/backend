...
public class MigrationClient {
	...
	// detailInfo
	public DetailInfoResponse getDetailInfoAccom(String contentId, String contentTypeId) {
		URI uri = UriComponentsBuilder.fromUriString(baseUrl + "/detailInfo")
			.queryParam("serviceKey", serviceKey)
			.queryParam("MobileOS", "ETC")
			.queryParam("MobileApp", "CatsGoToGedog")
			.queryParam("contentTypeId", ContentTypeId.숙박.getContentTypeId())
			.queryParam("_type", "json")
			.encode(StandardCharsets.UTF_8)
			.build()
			.toUri();
		log.info("API Request URL (detailInfo) : {}", uri);
		...
		try {
			return restClient.get()
				...
				.onStatus(HttpStatusCode::isError, (request, response) -> {
					String errorBody = new String(response.getBody().readAllBytes(), StandardCharsets.UTF_8);
					log.error("detailInfo API 요청중 오류 발생(컨텐츠 ID: {}) ({} {}) : Status {}, Body {}", contentId, request.getMethod(), request.getURI(), response.getStatusCode(), errorBody);
				})
				.body(DetailInfoResponse.class);
		}catch (Exception e) {
			log.error("detailInfo API 요청중 예상치 못한 오류 발생(컨텐츠 ID: {}) : {}", contentId, e.getMessage(), e);
			return null;
		}
	}

	// Content Image
	public List<DetailImageResponse.Item> getDetailImageList(int contentId) {
		URI uri = UriComponentsBuilder.fromUriString(baseUrl + "/detailImage")
			.queryParam("serviceKey", serviceKey)
			.queryParam("MobileOS", "ETC")
			.queryParam("MobileApp", "CatsGoToGedog")
		...
		log.info("API Request URL (detailImage) : {}", uri);
		...
		try {
			...
			return Optional.ofNullable(response)
				.map(DetailImageResponse::response)
				.map(DetailImageResponse.Response::body)
				.map(DetailImageResponse.Body::items)
				.map(DetailImageResponse.Items::item)
				.orElse(Collections.emptyList());
		} catch (RuntimeException e) {
			log.error("detailImage API 요청 중 오류 발생 (contentId: {}): {}", contentId, e.getMessage(), e);
			return Collections.emptyList();
		} catch (Exception e) {
			log.error("detailImage API 요청 중 예상치 못한 오류 발생 (contentId: {}): {}", contentId, e.getMessage(), e);
			return Collections.emptyList();
		}
	}
}
